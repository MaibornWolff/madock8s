...
func TestShouldStoreGeneratedMarkdownForGitHubUrl(t *testing.T) {
...
`
	...
	expItem := protocol.Markdown{
		...
		Exporter: "github",
		Tags:     []string{"readme", "github", "information", "gitlab"},
	}

	mockedGetMdFiles := func(map[string]string) ([]adapter.File, error) {
		return []adapter.File{
			{
				Name:    "Readme.md",
				Path:    "Readme.md",
	...
	mockedMdStorageServer := &mdstorage.MdstorageServerMock{
		StoreMarkdownFunc: func(ctx context.Context, md *protocol.Markdown) (*mdstorage.StoreResult, error) {
			document = *md
			return &mdstorage.StoreResult{Filename: "ExampleService GitHub"}, nil
		},
	}
